...
def write_csv(data, fieldnames, filename):
    with open(".\\regime1_csv\\" + filename + ".csv", mode="w", encoding='utf-8') as w_file:
...
if __name__ == '__main__':
    ...
    for i, r in enumerate(rads):
        print(i, "radius of", len(rads)-1)
        tot_table[:, 1:] = main_proc(orders, r, len(header) - 1)
        write_csv(tot_table, header, "order1sph"+str(r))
