...
def spectrum_freq(freqs):
    ...
    table = np.zeros((len(freqs), 13), dtype=complex)
...
if __name__ == '__main__':
    frequencies = np.linspace(1000, 20000, 1600, dtype=float)
    spectrum_table = np.zeros((len(frequencies), 14), dtype=complex)
    ...
    write_csv(spectrum_table, header, "spectrum_mltplecs_1aerogelPW")