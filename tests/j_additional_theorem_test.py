...
dist = np.array([-1, 1, 1])
desired_j = wvfs.regular_wave_function(m, n, x_p + dist[0], y_p + dist[1], z_p + dist[2], k_fluid)
srw_array = np.zeros(((order+1) ** 2, len(x_p)), dtype=complex)
i = 0
for munu in zip(wvfs.m_idx(order), wvfs.n_idx(order)):
    srw_array[i] = wvfs.regular_wave_function(munu[0], munu[1], x_p, y_p, z_p, k_fluid) * \
