...
def local_incident_coefficient(m, n, k, position, order):
...
def coefficient_array_inc_cls(order, direction, coefficient, length):
...
def incident_coefficient_cls(m, n, direction):
    r""" Coefficients in decomposition of plane wave
    d^m_n - eq(4.40) of 'Encyclopedia' """
...
def regular_wave_function_cls(m, n, x, y, z, k):
    r""" Regular basis spherical wave function
    ^psi^m_n - eq(between 4.37 and 4.38) of 'Encyclopedia' """
    r, phi, theta = mths.dec_to_sph(x, y, z)
...
def regular_wave_functions_array_cls(order, x, y, z, k):
    r""" builds np.array of all regular wave functions with n <= order"""
    rw_array = np.zeros(((order + 1) ** 2, len(x)), dtype=complex)
    i = 0
    for mn in multipoles(order):
        rw_array[i] = regular_wave_function_cls(mn[0], mn[1], x, y, z, k)
        i += 1
    return rw_array


def outgoing_wave_function_cls(m, n, x, y, z, k):
    r""" Outgoing basis spherical wave function
    psi^m_n - eq(between 4.37 and 4.38) in 'Encyclopedia' """
    r, phi, theta = mths.dec_to_sph(x, y, z)
...
def outgoing_wave_functions_array_cls(order, x, y, z, k):
    r""" builds np.array of all outgoing wave functions with n <= order"""
    ow_array = np.zeros(((order + 1) ** 2, len(x)), dtype=complex)
    i = 0
    for mn in multipoles(order):
        ow_array[i] = outgoing_wave_function_cls(mn[0], mn[1], x, y, z, k)
        i += 1
    return ow_array


def local_incident_coefficient_cls(m, n, k, direction, position, order):
    r""" Counts local incident coefficients
    d^m_nj - eq(42) in 'Multiple scattering and scattering cross sections P. A. Martin' """
    incident_coefficient_array = np.zeros((order+1) ** 2, dtype=complex)
    i = 0
    for munu in multipoles(order):
        incident_coefficient_array[i] = incident_coefficient_cls(munu[0], munu[1], direction) * \
                                        regular_separation_coefficient_cls(munu[0], m, munu[1], n, k, position)
        i += 1
    return mths.complex_fsum(incident_coefficient_array)


def axisymmetric_outgoing_wvf_cls(n, x, y, z, k):
    r, phi, theta = mths.dec_to_sph(x, y, z)
    return mths.sph_hankel1(n, k * r) * scipy.special.lpmv(0, n, np.cos(theta))


def axisymmetric_outgoing_wvf_array_cls(x, y, z, k, length, order):
    as_ow_array = np.zeros((order + 1, length), dtype=complex)
    for n in range(order + 1):
        as_ow_array[n] = axisymmetric_outgoing_wvf_cls(n, x, y, z, k)
    return as_ow_array


def regular_separation_coefficient_cls(m, mu, n, nu, k, dist):
    r""" Coefficient ^S^mmu_nnu(b) of separation matrix
    eq(3.92) and eq(3.74) in 'Encyclopedia' """
    if abs(n - nu) >= abs(m - mu):
        q0 = abs(n - nu)
    ...
    i = 0
    for q in range(0, q_lim + 1):
        sum_array[i] = (-1) ** q * regular_wave_function_cls(m - mu, q0 + 2 * q, dist[0], dist[1], dist[2], k) * \
                       mths.gaunt_coefficient(n, m, nu, -mu, q0 + 2 * q)
        i += 1
    return 4 * np.pi * (-1) ** (mu + nu + q_lim) * mths.complex_fsum(sum_array)


def outgoing_separation_coefficient_cls(m, mu, n, nu, k, dist):
    r""" Coefficient S^mmu_nnu(b) of separation matrix
    eq(3.97) and eq(3.74) in 'Encyclopedia' """
    if abs(n - nu) >= abs(m - mu):
        q0 = abs(n - nu)
    ...
    i = 0
    for q in range(0, q_lim + 1):
        sum_array[i] = (-1) ** q * outgoing_wave_function_cls(m - mu, q0 + 2 * q, dist[0], dist[1], dist[2], k) * \
                       mths.gaunt_coefficient(n, m, nu, -mu, q0 + 2 * q)
        i += 1
    return 4 * np.pi * (-1) ** (mu + nu + q_lim) * mths.complex_fsum(sum_array)
