...
def r_matrix(particles, layer, medium, freq, order, order_approx=1):
    r"""Build R matrix - reflection matrix"""
    block_matrix = np.zeros((len(particles), (order + 1) ** 2, (order + 1) ** 2), dtype=complex)
    omega = 2 * np.pi * freq
    a, alpha = reflection.ref_coef_approx(omega, medium.speed_l, layer.speed_l, medium.rho, layer.rho, order_approx, -3)
...
def layer_inc_coef_origin(incident_field, layer, order):
    ...
    for m, n in wvfs.multipoles(order):
        ...
        inc_coef_origin[n ** 2 + n + m] = wvfs.incident_coefficient(m, n, incident_field.dir) + \
                                          wvfs.local_incident_coefficient(m, n, incident_field.k_l, ref_dir, -image_o, order)
